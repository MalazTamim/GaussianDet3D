...
def main():
    ...
    num_warmup = 5
    pure_inf_time = 0
    num_processed = 0  # track samples after warmup
    ...
    for i, data in enumerate(data_loader):
        ...
        torch.cuda.synchronize()
        elapsed = time.perf_counter() - start_time  # in seconds
        ...
        if i >= num_warmup:
            ...
            if (i + 1) % args.log_interval == 0:
                avg_ms = (pure_inf_time / num_processed) * 1000
                print(f'Done image [{i + 1:<3}/ {args.samples}], '
                      f'current: {elapsed_ms:.2f} ms, '
                      f'avg: {avg_ms:.2f} ms')

        if (i + 1) == args.samples:
            avg_ms = (pure_inf_time / num_processed) * 1000
            print(f'\n=== Benchmark Results ===')
            print(f'Samples (excluding {num_warmup} warmup): {num_processed}')
            print(f'Average inference time: {avg_ms:.2f} ms')
            print(f'Equivalent FPS: {1000/avg_ms:.1f} img/s')
            break
