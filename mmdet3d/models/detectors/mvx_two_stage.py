...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def forward_train(self,
        ...
        """
        # print("forward_train mvx_two_stage.py")
        img_feats, pts_feats = self.extract_feat(
            points, img=img, img_metas=img_metas)
        losses = dict()
        
        # print("Debugging forward_pts_train:")
        # print("pts_feats:", pts_feats)
        # print("pts_feats shaoe:", pts_feats.shape)
        # print("type of pts_feats:", type(pts_feats))
        # if isinstance(pts_feats, (tuple, list)):
        #     for i, feat in enumerate(pts_feats):
        #         print(f"pts_feats[{i}] shape: {feat.shape}")
        # elif isinstance(pts_feats, torch.Tensor):
        ...
        if pts_feats:
            # print("pts_feats forward_pts_train:")
            losses_pts = self.forward_pts_train(pts_feats, gt_bboxes_3d,
                                                gt_labels_3d, img_metas,
                                                gt_bboxes_ignore)
            losses.update(losses_pts)
        if img_feats:
            print("img_feats img_train:")
