...
class SingleStageFSDV2(SingleStage3DDetector):
    ...
    def forward_train(self,
        ...
        # Debug: Check points shape
        # print(f"Points shape before segmentor: {points.shape}")

        # # If points has 4 dimensions, reshape it
        # if len(points.shape) == 4:
        #     # Assuming shape is (batch, num_gaussians, num_points, features)
        #     # Reshape to (batch, num_gaussians * num_points, features)
        #     batch_size = points.shape[0]
        #     points = points.reshape(batch_size, -1, points.shape[-1])
...
# seg_out_dict = self.segmentor(...)
        ...
        #     print(f"Points is a list with {len(points)} elements")
        #     print(f"First element shape: {points[0].shape}")
        #     print(f"Second element shape: {points[1].shape}")
        #     print(f"3rd element shape: {points[2].shape}")

        #     print(f"4th element shape: {points[3].shape}")
        #     print(f"8th element shape: {points[7].shape}")
        ...
        #     # Assuming each element has shape (num_points, features)
        #     # Stack them to get (batch_size, num_points, features)
        #     points = torch.stack(points, dim=0)
        #     print(f"After stacking, points shape: {points.shape}")
