...
class DeformableFeatureAggregation(nn.Module):#BaseModule
    ...
    def forward(
        ...
        **kwargs: dict,
        ...
        # Suppose this is your current lidar2img:
        # lidar2img = [[tensor(...), tensor(...), ..., tensor(...)]]  # outer list of 1, inner list of 6
        '''
        # Step 1: flatten the outer list
        if isinstance(lidar2img, list) and isinstance(lidar2img[0], list):
            lidar2img = lidar2img[0]  # unwrap the outer list if needed

        # Step 2: stack and convert to desired dtype/device
        projection_mat = torch.stack(lidar2img, dim=0)            # (6, 1, 4, 4)
        projection_mat = projection_mat.squeeze(1)                # (6, 4, 4)
        projection_mat = projection_mat.to(dtype=torch.float32)   # match model dtype

        # Step 3: add batch dimension
        projection_mat = projection_mat.unsqueeze(0)              # (1, 6, 4, 4)
        ...
        '''
        projection_mat = torch.stack(lidar2img, dim=1).to(dtype=torch.float32)   # (n, 1, 4, 4)
        # lidar2img = lidar2img.squeeze(1).to(torch.float32)    # (n, 4, 4)
        # print("lidar2img :", lidar2img)
        # print("lidar2img shape :", lidar2img.shape)

        # projection_mat = lidar2img.inverse().to(dtype=torch.float32) #, device=feature.device
        ...
        # print("metas at the start of forward deformable", img_metas)
        # print("lidar2img shape:", lidar2img.shape)
