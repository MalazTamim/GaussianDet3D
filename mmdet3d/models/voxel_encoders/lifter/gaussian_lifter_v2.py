...
class GaussianLifterV2(BaseLifter):
    def __init__(
        ...
        **kwargs,
        ...
        if pretrained_path is not None:
            #pretrained_path is wrong so lets update it
            print("pretrained_path  ", pretrained_path)
            # pretrained_path = 'GSFormerChkpts/Prob-64/state_dict.pth'
            # print("pretrained_path  ", pretrained_path)
    ...
    def forward(self, metas, **kwargs):
        if self.initialize_backbone is not None:
            # to fix: b, n = kwargs["imgs"].shape[:2] AttributeError: 'list' object has no attribute 'shape'
            if isinstance(kwargs["imgs"], list):
                kwargs["imgs"] = torch.stack(kwargs["imgs"], dim=0)
            if kwargs["imgs"].ndim == 6:
                kwargs["imgs"] = kwargs["imgs"].squeeze(1)
        ...
        raw = kwargs['lidar2img']
        import os; _dbg = os.environ.get('DEBUG_L2I', '0') == '1'
        if _dbg:
            elem = raw[0] if not isinstance(raw[0], list) else raw[0][0]
            print(f"[DEBUG lidar2img] type(raw)={type(raw)}, len={len(raw)}, "
                  f"type(raw[0])={type(raw[0])}, "
                  f"type(elem)={type(elem)}, "
                  f"shape={getattr(elem, 'shape', '?')}")
        if isinstance(raw[0], list):
            # Format: List[b][List[n][Tensor(4, 4)]]
            lidar2img = torch.stack(
                [torch.stack(mats, dim=0) for mats in raw], dim=0
            ).to(torch.float32)  # (b, n, 4, 4)
        else:
            # Format: List[n][Tensor(b, 4, 4)]
            lidar2img = torch.stack(raw, dim=1).to(torch.float32)  # (b, n, 4, 4)

        # Squeeze any spurious extra dims introduced by test-time collation (e.g. (b,n,1,4,4) -> (b,n,4,4))
        while lidar2img.ndim > 4:
            lidar2img = lidar2img.squeeze(2)

        # Invert per-sample per-camera: lidar2img -> img2lidar
        b_n, four1, four2 = lidar2img.shape[0] * lidar2img.shape[1], lidar2img.shape[2], lidar2img.shape[3]
        projection_mat = torch.inverse(
            lidar2img.reshape(b_n, four1, four2).cpu()
        ).reshape(b, lidar2img.shape[1], four1, four2).to(device=feature.device)  # (b, n, 4, 4)

        image_wh= torch.tensor([[[1600.,  864.],
                          [1600.,  864.],
                          [1600.,  864.],
                          [1600.,  864.],
                          [1600.,  864.],
                          [1600.,  864.]]], device=feature.device)
        ...
        uv = uv[None, None].expand(b, n, h, w, 2) * image_wh[:, :, None, None] # b, n, h, w, 2
        ...
        for i in range(b):
            ...
            if self.random_sampling:
                ...
                # scan = scan[scanidx, :] 
                scan = scan[scanidx.to(torch.long), :]
