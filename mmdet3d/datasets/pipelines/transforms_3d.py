...
import mmcv#needed for NormalizeMultiviewImage
from numpy import random

@PIPELINES.register_module()
class NormalizeMultiviewImage:
    """Normalize the multi-view images.
    ...
    def __repr__(self):
        return (
            f"{self.__class__.__name__}(mean={self.mean}, std={self.std}, to_rgb={self.to_rgb})"
        )
...
class ResizeCropFlipImage(object):
    """Resize/crop/flip/rotate multi-view images and update lidar2img/ego2img matrices.

    Ported from the official GaussianFormer codebase to keep the online image
    preprocessing byte-equivalent to the pipeline that generated the .bin files.

    Two usage modes:
      1. aug_configs already set in results (legacy): uses them as-is.
      2. Pass H, W, final_dim here: samples aug_configs internally per-call.
    """

    def __init__(self, H=None, W=None, final_dim=None,
    ...
    def __call__(self, results):
        ...
        if aug_configs is None:
            if self.H is None or self.W is None or self.final_dim is None:
                return results
...
@PIPELINES.register_module()
class LoadPrevMultiViewImageFromFiles(object):
    """Load, resize and normalize up to max_prev_frames previous multi-view frames.

    Reads 'prev_frames_data' from results (list of per-frame dicts populated by
    get_data_info). For each available frame it loads and preprocesses the 6
    camera images, then stacks everything into a single DC tensor of shape
    (max_prev_frames, N_cams, C, H, W).  Unavailable slots are zero-padded.

    The corresponding metadata list is stored in results['prev_frames_meta']
    for later collection into img_metas.

    Args:
        to_float32 (bool): Convert to float32. Default: True.
        color_type (str): Color type for mmcv.imread. Default: 'unchanged'.
        target_size (tuple): (W, H) target size for cv2.resize.
        mean / std / to_rgb: Normalisation matching the current-frame pipeline.
        num_cams (int): Number of cameras per frame. Default: 6.
        max_prev_frames (int): Fixed output depth (zero-padded). Default: 3.
    """
    ...
    def _load_frame(self, filenames, target_w, target_h):
        """Load one frame's camera images → (N_cams, C, H, W) float32 array."""
        imgs = []
        for fname in filenames:
            ...
            imgs.append(np.ascontiguousarray(img.transpose(2, 0, 1)))
    ...
    def __call__(self, results):
        ...
        prev_frames_data = results.get('prev_frames_data', [])

        # Build fixed-size (max_prev_frames, N_cams, C, H, W) tensor
        all_frames = []
        prev_frames_meta = []   # metadata list for the model; len = actual available

        for frame_data in prev_frames_data[:self.max_prev_frames]:
            frame_imgs = self._load_frame(frame_data['img_filenames'], target_w, target_h)
            all_frames.append(frame_imgs)
            prev_frames_meta.append({
                'lidar2img':              frame_data['lidar2img'],
                'ego2global_rotation':    frame_data['ego2global_rotation'],
                'ego2global_translation': frame_data['ego2global_translation'],
                'lidar2ego_rotation':     frame_data['lidar2ego_rotation'],
                'lidar2ego_translation':  frame_data['lidar2ego_translation'],
                'timestamp':              frame_data['timestamp'],
            })

        # Zero-pad to max_prev_frames so the DataLoader always sees the same shape
        n_valid = len(all_frames)
        while len(all_frames) < self.max_prev_frames:
            all_frames.append(np.zeros((self.num_cams, 3, target_h, target_w), dtype=np.float32))

        stacked = np.stack(all_frames, axis=0).astype(np.float32)  # (max_prev, N, C, H, W)
        results['prev_img'] = DC(to_tensor(stacked), stack=True)
        results['num_prev_frames'] = n_valid          # how many slots are real
        results['prev_frames_meta'] = prev_frames_meta  # list of dicts, len=n_valid
        return results

    def __repr__(self):
        return (
            f'{self.__class__.__name__}('
            f'max_prev_frames={self.max_prev_frames}, '
...
class NuScenesAdaptor(object):
    ...
    def __call__(self, input_dict):
        ...
        # print(f"img_shape: {input_dict['img_shape']}, type: {type(input_dict['img_shape'])}, shape: {np.array(input_dict['img_shape']).shape}")

        # input_dict["image_wh"] = np.ascontiguousarray(
        #     np.array(input_dict["img_shape"], dtype=np.float32)[:, :2][:, ::-1]
        # )
        # print("input_dict   ",input_dict)#IMPORTANT THIS CONTAINS ALL THE DATA PER SAMPLE APPARENTLY
        return input_dict
