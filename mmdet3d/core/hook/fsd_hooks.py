...
from mmdet.datasets import ConcatDataset   # <-- add this import


@HOOKS.register_module()
class DisableAugmentationHook(Hook):
    """Disable some data augmentations in the last N epochs.
    Edited so that it can handle ConcatDataset.

    Args:
        num_last_epochs (int): Number of epochs at the end of training during
            which the augmentations are disabled.
        skip_type_keys (tuple[str]): Pipeline step types to disable.
        dataset_wrap (bool): Whether each dataset is wrapped (e.g. CBGSDataset)
            and the real dataset is in `.dataset`.
    """
    ...
    def _update_dataset_pipeline(self, ds):
        """Call update_skip_type_keys on the innermost dataset if available."""
        # If wrapped (e.g. CBGSDataset), unwrap once
        if self.dataset_wrap and hasattr(ds, 'dataset'):
            ds = ds.dataset

        if hasattr(ds, 'update_skip_type_keys'):
            ds.update_skip_type_keys(self.skip_type_keys)

    def before_train_epoch(self, runner):
        epoch = runner.epoch  # begins from 0
        ...
        if epoch == runner.max_epochs - self.num_last_epochs:
            ...
            # Handle ConcatDataset (your case)
            if isinstance(ds, ConcatDataset):
                for sub_ds in ds.datasets:
                    self._update_dataset_pipeline(sub_ds)
            else:
                # Single dataset case (no concat)
                self._update_dataset_pipeline(ds)

            # Handle persistent workers hack
            ...
            # Once the restart is complete, restore the init flag.
